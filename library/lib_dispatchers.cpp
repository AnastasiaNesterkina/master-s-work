...
void SendTask(MPI_Status &st, MPI_Comm &CommWorker, MPI_Comm &CommMap, THREAD thread){
	...
	if (GetTask(&t)) {
		...
		MPI_Send(&send, 1, MPI_INT, peer, 2002, CommWorker, thread);
		...
		for (int j = 0; j < size_new; j++) {
			if (j != rank) {
				MPI_Send(&to_map_message, 2, MPI_INT, j, 1030, CommMap, thread);
			}
			else map[taskNumber] = peer;
		}
		
	}	// Send the message about task failure
	else MPI_Send(&send, 1, MPI_INT, peer, 2002, CommWorker, thread);
}
#else
...
void* dispatcher_old(void* me) {
	...
	while (!close) {
		MPI_Status st;
		#ifdef PROFILER
			MPI_Recv(&cond, 1, MPI_INT, MPI_ANY_SOURCE, 2001, oldComm_, &st, OldDispatcher);			
			// Task request
...
void* dispatcher(void* me) {
	...
	while (!close) {
		...
		#ifdef PROFILER				
			MPI_Recv(&cond, 1, MPI_INT, MPI_ANY_SOURCE, 2001, Comm, &st, Dispatcher);
			// Task request
		...
		// Communicator is changing
			...
			int to_map_message[2] = { cond, cond };
				...
				MPI_Send(&to_map_message, 2, MPI_INT, rank, 1030, currentComm, Dispatcher);
			...
			MPI_Comm_size(Comm, &size_new);
				...
				if (rank == 0) {
					...
					for (int k = size_old; k < size_new; k++) {
						MPI_Send(&sizeOfMap, 1, MPI_INT, k, 10000, newComm, Dispatcher);
						MPI_Send(map.data(), map.size(), MPI_INT, k, 10001, newComm, Dispatcher);
					}
				}
				MPI_Barrier(currentComm, Dispatcher);
			#else					
			...
			cond = 2;
			#ifdef PROFILER
				MPI_Send(&cond, 1, MPI_INT, rank_old, 1999, oldComm_, Dispatcher);			
